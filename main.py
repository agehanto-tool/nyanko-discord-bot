...
@app_commands.describe(
    target="価格変更の対象範囲",
    price="新しい価格（円）",
    category="カテゴリを選択（対象が特定のカテゴリの場合）",
    item_key="アイテムキーを直接入力（対象が特定のものだけの場合）"
)
...
@app_commands.choices(category=[
    ...
    app_commands.Choice(name="ステージ系", value="stage"),
    app_commands.Choice(name="キャラクター系", value="chara"),
    app_commands.Choice(name="その他", value="etc"),
])
async def change_price(
    ...
    if target_value == "category" and category is None:
        embed = error_embed("エラー", "特定のカテゴリだけを選択した場合はカテゴリを指定してください。")
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return

    if target_value == "specific" and item_key is None:
        embed = error_embed("エラー", "特定のものだけを選択した場合はアイテムキーを指定してください。\n例: item_key:catfood_50000")
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
...
class NyankoServiceSelect(View):
    def __init__(self, user):
        ...
        next_btn = Button(label="次へ", style=discord.ButtonStyle.success, custom_id="next_step")
